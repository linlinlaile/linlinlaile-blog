...
function handle422Error(body?: string): void {
	console.error('GitHub 422:', body || '')
	toast.error('422: ' + (body || '操作太快了，请操作慢一点'))
}
...
export async function updateRef(token: string, owner: string, repo: string, ref: string, sha: string, force = false): Promise<void> {
	...
	if (res.status === 422) {
		...
		if (body) {
			console.error('GitHub 422:', JSON.stringify(body, null, 2))
			throw new Error(`update ref: ${body.message}`)
		}
	}
